...
function splitMessagesIntoDays(messages: MessageType[]): MessageType[][] {
  return messages.reduce(
    (
      ...
      const prevDate =
        arr[id - 1] && new Date(arr[id - 1].timestamp).toLocaleDateString("sv");
...
function formatDay(timestamp: string): string {
  ...
  if (date.toLocaleDateString("sv") === today.toLocaleDateString("sv")) {
    ...
    date.toLocaleDateString("sv") === yesterday.toLocaleDateString("sv")
  ) {
    return "Yesterday";
  ...
  return (
    <ul className="max-w-[40rem] m-auto">
      {splitMessagesIntoDays(messages).map((day) => (
        <Day key={day[0].timestamp} messages={day} />
      ))}
    </ul>
  );
}

function Day({ messages }: MessageListProps) {
  return (
    <div className="pt-4">
      ...
      {messages.map((msg, id) => (
        <Message
          key={msg._id}
          message={msg}
          followed={checkFollowed(msg, messages[id + 1])}
        />
      ))}
    </div>
  );
