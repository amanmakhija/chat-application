...
export default function ChatList() {
  ...
  const filterChats = (chat: Chat) =>
    chat.messages.length > 0 || chat._id === chatId;
...
function ChatTab({ chat }: ChatTabProps) {
  ...
  const otherMember = chat.members.filter((user) => user._id !== authUser?._id);
  const lastMessage = chat.messages[chat.messages.length - 1];
  const isTyping = chat.typingUsers.filter(
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold truncate">
              {getChatName(otherMember)}
            </h3>
            <div
              className={`${
                chat.unreadCount ? "text-white font-medium" : "text-neutral-400"
              } `}
            >
              {isTyping.length ? (
                    ...
                    <div className="flex-1 min-w-0 truncate">
                      {lastMessage.images.length > 0 && !lastMessage.text && (
                        <span className="italic">
                          {lastMessage.images.length > 1 ? "images" : "image"}
                        </span>
                      )}
                      {lastMessage.text}
                    </div>
          ...
          {lastMessage && (
            <div className="space-y-1">
              <p
                className={`text-xs ${
                  chat.unreadCount ? "text-white" : "text-neutral-400"
                }`}
              >
...
function ChatAvatar({ chat }: ChatAvatarProps) {
  ...
  return (
    <div className="relative">
      ...
      {otherMembers[0].isOnline && <OnlineBadge selected={selected} />}
    </div>
  );
}
    ...
    <div className="w-fit h-fit absolute right-0 bottom-0">
      ...
      >
        <circle
          ...
          cy="8"
          r="8"
          className={`fill-neutral-950 group-hover:fill-neutral-900 transition-all ${
            selected && "fill-neutral-900"
          }`}
        />
